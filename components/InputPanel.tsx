...
import { ValidationReport, AutoFixReport } from '../types';
import { ValidationReportView } from './ValidationReportView';
import { AutoFixReportView } from './AutoFixReportView';
import { LoaderCircle } from './icons';
import { SAMPLE_OPENAPI_JSON, SAMPLE_POSTMAN_JSON } from '../constants';
...
export const InputPanel: React.FC<InputPanelProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-slate-200/50 dark:border-slate-800/50 space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">1. Input API Specification</h2>
        <select onChange={handleSampleSelect} className="text-sm border border-slate-300 dark:border-slate-700 rounded-md bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 py-1 px-2 focus:ring-1 focus:ring-indigo-500">
            <option value="openapi">Sample: OpenAPI</option>
            <option value="postman">Sample: Postman</option>
        </select>
      </div>
      <textarea
        value={specInput}
      ...
      <div className="flex items-center space-x-3">
        ...
        >
          {isProcessing && !isFixing ? <LoaderCircle className="animate-spin mx-auto" /> : 'Validate Spec'}
        </button>
        <button
          onClick={onAutoFix}
        ...
        >
          {isFixing ? <LoaderCircle className="animate-spin mx-auto" /> : 'Auto-Fix with AI'}
        </button>
      </div>
      {autoFixReport && <AutoFixReportView report={autoFixReport} />}
      {validationReport && <ValidationReportView report={validationReport} />}
    </div>
  );
};