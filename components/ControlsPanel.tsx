...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900/70 backdrop-blur-sm p-6 rounded-2xl shadow-lg border border-slate-200/50 dark:border-slate-800/50">
      <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">2. Generation Controls</h2>
          ...
          {Object.entries(AUDIENCE_CONFIG).map(([key, { name, description }]) => (
            ...
            >
              <div className="flex-grow">
               ...
               <div className={`w-5 h-5 rounded-full border-2 ml-4 flex-shrink-0 flex items-center justify-center transition-all duration-200 ${selectedAudiences.includes(key) ? 'bg-indigo-600 border-indigo-600' : 'border-slate-400 bg-white dark:bg-slate-700'}`}>
                {selectedAudiences.includes(key) && <div className="w-2 h-2 rounded-full bg-white"></div>}
               </div>
            </button>
          ))}
        </div>
      ...
      <div className="mt-6">
        ...
        {isGenerationDisabled && !isLoading && <p className="text-xs text-center mt-2 text-slate-500">Please validate the spec successfully before generating.</p>}
      </div>
    </div>
  );
};