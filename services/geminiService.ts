...
import { RawDrafts, N_API } from "../types";
import { RESPONSE_SCHEMA, GENERATE_DRAFTS_INSTRUCTION } from "../constants";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

function cleanJsonString(jsonString: string): string {
    // Remove potential markdown code fences and trailing commas
    let cleaned = jsonString.replace(/```json/g, '').replace(/```/g, '').trim();
    cleaned = cleaned.replace(/,\s*([}\]])/g, '$1');
    return cleaned;
}

export async function generateRawDrafts(n_api: N_API, audiences: string[]): Promise<RawDrafts> {
  // Fix: Cast `dynamicSchema` to `any` to allow adding the 'required' property.
  const dynamicSchema: any = { ...RESPONSE_SCHEMA };
  // Only require the audiences that were requested
  ...
  try {
    ...
    try {
        ...
        return parsedJson as RawDrafts;
    } catch (e) {
